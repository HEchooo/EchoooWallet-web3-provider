...
const EchoooWallet = {
  init(rpcUrl, options, syncOptions) {
    ...
    if (typeof username === "undefined" || typeof password === "undefined") {
      engine.addProvider(
        new Web3Subprovider(new Web3.providers.HttpProvider(rpcUrl))
      );
    } else {
      engine.addProvider(
        new Web3Subprovider(
          new Web3.providers.HttpProvider(rpcUrl, 0, username, password)
        )
      );
    }
...
ProviderEngine.prototype.send = function (payload) {
  ...
  switch (payload.method) {
    ...
    case "net_listening":
      try {
        self._providers
          .filter((p) => p.provider !== undefined)[0]
  ...
  switch (payload.method) {
    ...
    case "eth_chainId":
      var result = {
        ...
        jsonrpc: payload.jsonrpc,
        result:
          "0x" + parseInt(globalSyncOptions.networkVersion).toString(16) ||
          null,
      };
      cb(null, result);
      break;
